...
/*
  this is just stupid - there is no way in JS to have a catch-all getter/setter like in
  Python. This leads to an evil hack observing object's properties every TM milliseconds
  and post-reacting on new ones adding them to implant's communication.
  This can be "fixed" when we have a list of those variables a conjoiner is interested
  in - having fixed names will allow to use __defineGetter__ / Setter.
  Right now, it has a latency up to 1 sec to know a new value - an unknown or even a
  known one.

  It is thus better to have declared fields in the class that will be hooked instead of
  working with completely dynamic ones.

  What also can be done: consider only declared fields as "interested in" and just ignore
  the rest.
*/
var TM = 1000;
...
exports.implant = function(o, cfg_file, my_name, callback) {
    fs.readFile(cfg_file, { encoding: 'utf8' }, function(err, data) {
        if (err) throw err;
        ...
        var myUrl = function() {
            for (var c in conf.conjoiners) {
                if (conf.conjoiners[c].name == my_name) {
        ...
        var packPayloadSingle = function(n, v) {
            var ret = {"sender": my_name, "time": new Date().getTime()};
            ret[keyN(n)] = v;
        ...
        var ensureHook = function(n) {
            ...
            o.__defineGetter__(n, function() {
                ...
                // so there is no need to coordinate through a queue or anyhow at all
            ...
            for (var c in conf.conjoiners) {
                if (conf.conjoiners[c].name != my_name) {
                    ...
                    con.on('message', function(payload) {
                        ...
                        var res = unpackPayloadSingle(opayload); // later: internalize time

                        // in case the variable is unknown, hook it first
                        ensureHook(res.n);

                        // set value directly to avoid the setter, thus multicast
        ...
        /*
          use recursive setTimout instead of setInterval, because it's soft on busy servers
          and only will call the function when the event comes through, not at any cost even
          when events can't be handled.
        ...
        o[TMLOOP] = function() {
            for (var k in o) {
                if ((o[KEYS].indexOf(k) == -1) &&
                    ...
                    /*
                      read the current value and queue it.
                      the current value will be multicasted to all other conjoiners.
                     */
                    o[k] = v;
                }
            }
