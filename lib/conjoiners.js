 ...
 * @function
 * @description
 * Emit a property change object on the conjoiner's EventEmitter.
 *
 * @param {Object} obj The implanted object
...
var emitOnTransenlightenment = function (obj, property) {
    ...
    obj.transenlightenment.emit(property, event);
    obj.transenlightenment.emit(event);
};
 ...
 * @description
 * @function
 * The standard Node.js EventEmitter is used by conjoiners.js, but
 * unfortunately it is lacking one crucial functionality: Observe all
 * events. This function creates a standard Node.js EventEmitter and monkey
 * patches it so that we one can call its functions without an event type.
 *
 * @returns {EventEmitter} A standard node.js EventEmitter with one addition:
 *   All functions can be called without an event type in order to listen to
 *   all property changes.
 */
var newAllEventPermittingEventEmitter = function () {
    var emitter = new EventEmitter();

    var permitAll = function (fn) {
        return function (type) {
            var args = arguments;

            // function called without an event type => listen to all events
            if (typeof type !== 'string') {
                // copy the arguments object so that we can modify it.
    ...
    emitter.on = emitter.addListener = permitAll(emitter.on);
    emitter.once = permitAll(emitter.once);
    emitter.emit = permitAll(emitter.emit);
    emitter.listeners = permitAll(emitter.listeners);
    emitter.setMaxListeners = permitAll(emitter.setMaxListeners);
    emitter.removeListener = permitAll(emitter.removeListener);
    emitter.removeAllListeners = permitAll(emitter.removeAllListeners);
...
var implantInternal = function (obj, config, name) {
    ...
    // We create an EventEmitter on the conjoined object. Sucks a bit as we
    // need to monkey patch the conjoined object, but it will enable a
    // comfortable programming model.
    obj.transenlightenment = newAllEventPermittingEventEmitter();
 ...
 * @param {String} name The conjoiner's name as defined in the configuration
 *      file.
 * @param {Function} [callback] An optional success / error handler. Signature:
 *      function (err) {}.
 * @return {Q.promise} A promise which is resolved once the initialisation is
 *      complete or when an error occurs. Conjoiners is using
 *      <a href='https://github.com/kriskowal/Q'>Q promises</a>.
 */
exports.implant = function (obj, configFilePath, name, callback) {
    return readConfigFile(configFilePath).then(function (config) {
        return implantInternal(obj, config, name).then(function () {
            if (callback != null) {
                callback(null);
            }
        });
        ...
        // propagate the error to further fail() handlers down
        // the chain.
        throw err;
    });
};
