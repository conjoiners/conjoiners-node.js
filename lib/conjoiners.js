...
exports.implant = function(o, cfgFile, myName, callback) {
    ...
    fs.readFile(cfgFile, { encoding: 'utf8' }, function(err, data) {
        ...
        // prepare PUB
        var ensureExternalBind = function(cb) {
            if (o[EXTS] == null) {
                var sock = zmq.socket('pub');
                sock.bind(myUrl(), function() {
                    ensureMulticast();
                    ensureConjoinersConnect(cb);
                });
                o[EXTS] = sock;
            } else {
                cb();
        ...
        // prepare multicasting
        var ensureMulticast = function() {
            if (o[EVENTER] == null) {
        ...
        // connect to conjoiners
        var ensureConjoinersConnect = function(cb) {
            if (o[CONS] == null) {
                o[CONS] = [];
                for (var c in conf.conjoiners) {
                    if (conf.conjoiners[c].name !== myName) {
                        ...
                        con.subscribe('');
                        //con.setsockopt(zmq.ZMQ_RCVTIMEO, conf.recv_timeout);
                        //damn it, the option isn't exposed to the user
        ...
        */
        ...
        o[KEYS] = [];
        o[VALS] = [];
        o[TMLOOP] = function(callback) {
            // wire global stuff
            ensureExternalBind(function() {
                for (var k in o) {
