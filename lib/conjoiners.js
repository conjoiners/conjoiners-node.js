 ...
 *      complete or when an error occurs. Conjoiners is using
 *      <a href='https://github.com/kriskowal/Q'>Q promises</a>. The promise is
 *      resolved with a {@link events.EventEmitter} which emits an 'update'
 *      event on every property change.
 */
        ...
        subConnections.forEach(function (subConnection) {
            subConnection.on('message', function (payload) {
                handleIncomingTransenlightenment(payload.toString(),
                    ensureHooked).forEach(function (changedProperty) {
                        eventEmitter.emit('update', {
                            property: changedProperty,
                            value: obj[changedProperty],
                            obj: obj
                        });
                    });
            });
